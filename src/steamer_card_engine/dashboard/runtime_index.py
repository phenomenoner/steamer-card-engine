...
RUNTIME_BUCKET = "lyria-trading-ops-975050019139-ap-east-2"
...
def _list_watchlist_dates(root: Path) -> set[str]:
    dates: set[str] = set()
    candidates = [root / "staging" / "steamer" / "watchlists", Path("/opt/trading/shared/watchlists")]
...
def build_runtime_dates_index(root: Path | None = None) -> dict[str, Any]:
    ...
    all_dates = sorted(set(local_counts) | set(s3_by_date) | set(fixture_by_date) | watchlist_dates, reverse=True)
    dates: list[dict[str, Any]] = []
    for date in all_dates:
        lanes = local_counts.get(date, {})
        local_run_count = sum(lanes.values())
        ...
        dates.append(
            {
                ...
                "preferred": local_run_count > 0 or bool(s3_state.get("s3_archive_present")) or date in watchlist_dates,
            }
        )

    return {
        "source_kind": "local-runs+s3-runtime-archive-cache+fixture-compare-index",
