...
- 2026-04-09 — strategy tab gained a thin read-only active-family baton line sourced from the existing proposal/active paired-lane artifacts. It surfaces active family, attached decks, handoff readiness, and explicit proposed-vs-active divergence without adding control authority. Topology unchanged.
- 2026-04-09 — `五氣朝元` stale-rule retirement sweep closed for the baton-line slice: repo AGENTS/operator setup wording were recut to the broader Steamer Dashboard truth; older build packets/checkpoints remain historical only; closure receipt recorded at `docs/receipts/2026-04-09_dashboard_active-family-baton-line_wuqi-chaoyuan_closure.md`. Topology unchanged.
- 2026-04-09 — landed a thin shared history-source index helper for the Strategy Powerhouse tab so proposal-day packet discovery, verifier refs, family-specific extras, and indexed baton-source lookup stop depending on hand-wired dated paths inside the dashboard view. Local-artifact/read-only only; topology unchanged.
