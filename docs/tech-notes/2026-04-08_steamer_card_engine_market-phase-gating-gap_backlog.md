...
## Implementation status — 2026-04-08 slice 2c

Landed in repo truth/code:
- emitted execution requests now also create a first `order-lifecycle.jsonl` placeholder receipt (`state=new`) so the forced-exit bridge no longer stops at execution shells alone
- dashboard phase summary now also carries `open_discovery_summary`, and Mission Control surfaces an operator-visible open-discovery metric instead of hiding that truth only in `run-manifest.json`
- synthetic tests now cover forced-exit lifecycle placeholder emission in addition to forced-exit execution-request emission

Still open after slice 2c:
- open-discovery still relies on event-hint presence rather than a richer explicit exchange/open-state model
- forced-close lifecycle still remains a placeholder bridge (no chunking/pacing/fill/position receipts)
- historical committed March fixtures do not retroactively gain new lifecycle/open-discovery fields unless regenerated
